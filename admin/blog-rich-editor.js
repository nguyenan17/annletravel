...
// Quill is loaded lazily only when the Blog editor is actually opened.
...
(function initBlogRichEditor() {
    ...
    function loadQuill(callback) {
        ...
        const finish = () => {
            quillLoading = false;
            const callbacks = quillCallbacks.splice(0);
        ...
        script.onerror = () => {
            ...
            quillCallbacks = [];
            console.error("Không tải được Quill rich text editor.");
            setStatus("Không tải được trình soạn thảo Blog. Kiểm tra kết nối mạng rồi thử lại.", "error");
        };
    ...
    function addStyles() {
        ...
        style.textContent = `.blog-rich-editor-shell{border:1px solid #d8e1e5;border-radius:10px;overflow:hidden;background:#fff}.blog-rich-editor-shell .ql-toolbar.ql-snow{border:0;border-bottom:1px solid #e3e9ed;background:#f8fafb;padding:9px 10px}.blog-rich-editor-shell .ql-container.ql-snow{border:0;font-family:inherit;font-size:16px}.blog-rich-editor-shell .ql-editor{min-height:390px;padding:18px;line-height:1.75}.blog-rich-editor-shell .ql-editor.ql-blank::before{color:#9aa6ad;font-style:normal}.blog-rich-editor-shell .ql-editor h2{margin:18px 0 8px;font-size:25px}.blog-rich-editor-shell .ql-editor h3{margin:16px 0 7px;font-size:20px}.blog-rich-editor-shell .ql-editor img{max-width:100%;height:auto;border-radius:10px}.blog-rich-editor-shell .ql-editor blockquote{border-left:4px solid #d8e1e5;padding-left:14px;color:#5e6b73}.blog-rich-editor-note{display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;margin-top:7px;font-size:12px;color:#81909a;line-height:1.5}.blog-rich-editor-note strong{color:#52616a}.blog-rich-editor-status{margin-top:7px;min-height:18px;font-size:12px;color:#66727d}.blog-rich-editor-status.error{color:#c0392b}.blog-rich-editor-status.success{color:#237a57}.blog-html-source{display:none!important}`;
    ...
    function getSafeFileName(file) {
        ...
        const base = file.name.replace(/\.[^/.]+$/, "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/đ/g, "d").replace(/Đ/g, "D").replace(/[^a-zA-Z0-9_-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60) || "blog-image";
    ...
    function getImageDimensions(file) {
        return new Promise((resolve, reject) => {
            ...
            image.onerror = () => { URL.revokeObjectURL(url); reject(new Error("Không đọc được kích thước ảnh.")); };
            image.src = url;
        });
    }
    ...
    async function uploadImage(file) {
        ...
        const path = `blog/${new Date().toISOString().slice(0, 10)}/${getSafeFileName(optimized)}`;
        setStatus("Đang tải ảnh lên...");
        const { error } = await client.storage.from(BLOG_IMAGE_BUCKET).upload(path, optimized, { cacheControl: "3600", contentType: optimized.type, upsert: false });
    ...
    function ensureEditor() {
        ...
        toolbar.innerHTML = `<span class="ql-formats"><select class="ql-header"><option value="2"></option><option value="3"></option><option selected></option></select></span><span class="ql-formats"><button class="ql-bold"></button><button class="ql-italic"></button><button class="ql-underline"></button><button class="ql-strike"></button></span><span class="ql-formats"><button class="ql-blockquote"></button></span><span class="ql-formats"><button class="ql-list" value="ordered"></button><button class="ql-list" value="bullet"></button><select class="ql-align"></select></span><span class="ql-formats"><button class="ql-link"></button><button class="ql-image" title="Tải ảnh lên"></button><select class="ql-color"></select><select class="ql-background"></select></span><span class="ql-formats"><button class="ql-clean"></button></span>`;
        ...
        quill = new Quill(editor, {
            ...
            modules: { toolbar: { container: "#blogRichEditorToolbar", handlers: { image: openImagePicker } } }
        });
    ...
    function initializeEditorWhenAvailable(html = "") {
        const textarea = document.getElementById("blogContent");
        if (!textarea) return;
        loadQuill(() => {
            const editor = ensureEditor();
            if (editor) setHtml(html || textarea.value || "");
        });
    }
    ...
    function wrapEditorOpen() {
        ...
        window.openEmbeddedBlogEditor = function (post) {
            ...
            const html = post?.content || document.getElementById("blogContent")?.value || "";
            setTimeout(() => initializeEditorWhenAvailable(html), 0);
        };
        wrapped = true;
    }
    ...
    function watchForBlogEditor() {
        ...
        // Không tải Quill khi Admin mới mở. Chỉ tải khi Blog editor thực sự xuất hiện.
        const tryInitialize = () => {
            wrapEditorOpen();
            const textarea = document.getElementById("blogContent");
            if (textarea && !quill) initializeEditorWhenAvailable(textarea.value || "");
        };
