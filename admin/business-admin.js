...
const notesTable = 'business_lead_notes';
const statusText = {new:'Mới', contacted:'Đã liên hệ', quoted:'Đã báo giá', won:'Đã chốt', lost:'Không chốt'};
const noteTypeText = {note:'Ghi chú', call:'Cuộc gọi', email:'Email', meeting:'Gặp mặt', quote:'Báo giá', follow_up:'Follow-up'};
...
window.openLead=async id=>{
 const x=leads.find(v=>v.id===id); if(!x)return;
 selectedLeadId=id; $('leadStatus').value=x.status;
 $('leadDetail').innerHTML=`<p><strong>${esc(x.company_name)}</strong> — ${esc(x.contact_name)}</p><p>📞 ${esc(x.phone)}${x.email?' · ✉️ '+esc(x.email):''}</p><p>👥 ${x.employee_count||'Chưa rõ'} người · ${esc(x.service_interest||'Chưa chọn dịch vụ')}</p><p>📍 ${esc(x.departure||'Chưa rõ')} → ${esc(x.destination||'Chưa rõ')}</p><p>📅 ${fmtDate(x.departure_date)}${x.return_date?' → '+fmtDate(x.return_date):''}</p><p>💰 ${esc(x.budget||'Chưa xác định')}</p><p style="margin-top:12px"><strong>Yêu cầu:</strong><br>${esc(x.message||'Không có')}</p>`;
 $('careNote').value=''; $('careFollowUp').value=''; $('leadModal').classList.remove('hidden'); await loadLeadNotes(id);
};
async function loadLeadNotes(leadId){
 const {data,error}=await supabaseClient.from(notesTable).select('*').eq('lead_id',leadId).order('created_at',{ascending:false});
 if(error){$('careList').innerHTML='<div class="biz-empty">Chưa cài lịch sử chăm sóc. Hãy chạy file supabase/business_lead_notes.sql.</div>';console.error(error);return;}
 const notes=data||[];
 $('careList').innerHTML=notes.length?notes.map(n=>`<article class="care-item"><div class="care-item-head"><span class="care-type">${noteTypeText[n.note_type]||'Ghi chú'}</span><span class="care-time">${fmtDateTime(n.created_at)}</span></div><div class="care-note">${esc(n.note)}</div>${n.follow_up_at?`<div class="care-follow">↗ Follow-up: ${fmtDateTime(n.follow_up_at)}</div>`:''}</article>`).join(''):'<div class="biz-empty">Chưa có lịch sử chăm sóc.</div>';
}
$('careForm').onsubmit=async e=>{
 e.preventDefault(); if(!selectedLeadId)return;
 const note=$('careNote').value.trim(); if(!note)return;
 const {data:{session}}=await supabaseClient.auth.getSession();
 const payload={lead_id:selectedLeadId,note,note_type:$('careType').value,follow_up_at:$('careFollowUp').value?new Date($('careFollowUp').value).toISOString():null,created_by:session?.user?.id||null};
 const {error}=await supabaseClient.from(notesTable).insert(payload);
 if(error){alert('Không thể thêm lịch sử: '+error.message);return;}
 $('careNote').value=''; $('careFollowUp').value=''; await loadLeadNotes(selectedLeadId);
};
