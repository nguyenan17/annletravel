...
function destinationAdminEscape(value) {
...
async function loadAdminDestinations() {
    ...
    if (error) {
        ...
        alert("Không thể tải danh sách điểm đến.\n\n" + error.message);
        return;
    }

    adminDestinations = data || [];
    renderAdminDestinations();
}

function renderAdminDestinations() {
    const body = document.getElementById("destinationTableBody");
    if (!body) return;

    if (!adminDestinations.length) {
        body.innerHTML = `<tr><td colspan="6">Chưa có điểm đến nào.</td></tr>`;
        return;
    }

    body.innerHTML = adminDestinations.map(item => `
        <tr>
            <td><strong>${destinationAdminEscape(item.name)}</strong><br><small>${destinationAdminEscape(item.slug)}</small></td>
            <td>${destinationAdminEscape(item.country)}</td>
            <td>${item.region === "international" ? "🌏 Nước ngoài" : "🇻🇳 Trong nước"}</td>
            <td>${item.featured ? "✓" : "-"}</td>
            <td>${Number(item.sort_order || 0)}</td>
            <td><div class="action-buttons"><button type="button" class="refresh-button" onclick="editDestination('${destinationAdminEscape(item.id)}')">Sửa</button><button type="button" class="booking-delete-button" onclick="removeDestination('${destinationAdminEscape(item.id)}')">Xóa</button></div></td>
        </tr>
    `).join("");
}
...
window.editDestination = function (id) {
    const destination = adminDestinations.find(item => String(item.id) === String(id));
    if (destination) openDestinationModal(destination);
};

window.removeDestination = async function (id) {
    const destination = adminDestinations.find(item => String(item.id) === String(id));
    if (!destination) return;

    if (!confirm(`Bạn có chắc muốn xóa điểm đến "${destination.name}"?`)) return;

    const { error } = await supabaseClient.from("destinations").delete().eq("id", id);
    if (error) {
        console.error("Delete destination error:", error);
        alert("Không thể xóa điểm đến.\n\n" + error.message);
        return;
    }
...
function createDestinationId(slug) {
    return slug.trim().toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-+|-+$/g, "") || `destination-${Date.now()}`;
}
...
async function saveDestination(event) {
    ...
    if (!name || !slug || !country) {
        alert("Vui lòng nhập tên, slug và quốc gia.");
        return;
    }
    ...
    if (editingDestinationId) {
        query = supabaseClient.from("destinations").update(payload).eq("id", editingDestinationId);
    } else {
        payload.id = createDestinationId(slug);
        query = supabaseClient.from("destinations").insert(payload);
    }

    const { error } = await query;
    if (error) {
        console.error("Save destination error:", error);
        alert("Không thể lưu điểm đến.\n\n" + error.message);
        return;
    }

    closeDestinationModal();
    alert(editingDestinationId ? "Cập nhật điểm đến thành công!" : "Thêm điểm đến thành công!");
    await loadAdminDestinations();
}
...
(function initDestinationAdmin() {
    ...
    const closeButton = document.getElementById("closeDestinationModalButton");
    const cancelButton = document.getElementById("cancelDestinationButton");
    const nameInput = document.getElementById("destinationName");
    const slugInput = document.getElementById("destinationSlug");

    form?.addEventListener("submit", saveDestination);
    addButton?.addEventListener("click", () => openDestinationModal());
    closeButton?.addEventListener("click", closeDestinationModal);
    cancelButton?.addEventListener("click", closeDestinationModal);
    nameInput?.addEventListener("blur", () => {
        if (!slugInput.value.trim()) slugInput.value = autoSlug(nameInput.value);
    });
