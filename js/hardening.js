...
(function () {
    ...
    if (typeof originalRenderTourDetail === "function") {
        window.renderTourDetail = async function () {
            ...
            if (!container) return originalRenderTourDetail();
            ...
            if (!tour) {
                ...
                container.innerHTML = `<div class="container section"><h2>Không tìm thấy tour.</h2><p>Tour bạn đang tìm kiếm không tồn tại hoặc đã được thay đổi.</p><br><a href="tours.html" class="btn btn-primary">Xem tất cả tour</a></div>`;
                return;
            }
            ...
            try {
                ...
                container.innerHTML = data.map(item => `
                    <div class="itinerary-day">
                        ...
                        ${item.description ? `<p>${escapeHtmlSafe(item.description)}</p>` : ""}
                    </div>`).join("");
            } catch (error) {
    ...
    if (typeof originalCreateTourCard === "function") {
        window.createTourCard = function (tour) {
            ...
            return `<article class="tour-card"><a href="tour-detail.html?id=${id}" class="tour-image" style="background-image: url('${escapeCssUrl(image)}')"><span class="tour-location">${destination}</span><span class="tour-seats">Còn ${seats} chỗ</span></a><div class="tour-content"><div class="tour-date">📅 ${escapeHtmlSafe(tour.displayDate)}</div><h3>${name}</h3><p>${short}</p><div class="tour-meta"><span>📍 ${destination}</span><span>👥 ${seats} chỗ</span></div><div class="tour-footer"><div class="tour-price"><small>Chỉ từ</small><strong>${formatPrice(price)}</strong></div><a href="tour-detail.html?id=${id}" class="tour-button">Xem tour <span>→</span></a></div></div></article>`;
        };
    }
    ...
    if (typeof originalRenderMonthlyTours === "function") {
        window.renderMonthlyTours = async function () {
            ...
            const monthlyTours = data.filter(tour => {
                const departureKey = String(tour.departure ?? "").slice(0, 10);
                return /^\d{4}-\d{2}-\d{2}$/.test(departureKey) && departureKey >= todayKey;
            }).sort((a, b) => String(a.departure ?? "").slice(0, 10).localeCompare(String(b.departure ?? "").slice(0, 10))).slice(0, 4);
            if (monthlyTours.length === 0) {
                container.innerHTML = `<div class="departure-empty"><p>Hiện chưa có tour nào có lịch khởi hành sắp tới.</p><a href="tours.html" class="btn btn-primary">Xem tất cả tour</a></div>`;
                return;
            }
            container.innerHTML = monthlyTours.map(tour => `<div class="departure-row"><div><strong>${escapeHtmlSafe(tour.displayDate)}</strong><small>${escapeHtmlSafe(tour.destination)}</small></div><strong>${escapeHtmlSafe(tour.name)}</strong><span>Còn ${Number(tour.seats) || 0} chỗ</span><strong>${formatPrice(Number(tour.price) || 0)}</strong><a href="tour-detail.html?id=${encodeURIComponent(String(tour.id ?? ""))}">Xem tour</a></div>`).join("");
        };
    }
    ...
    if (document.body.classList.contains("personal-page") || document.getElementById("allTours")) {
    ...
    function renderHeaderSocial() {
        if (document.querySelector(".header-social")) return;
        ...
        social.innerHTML = `
            <a class="header-social-link" href="https://web.facebook.com/profile.php?id=61550505475800" target="_blank" rel="noopener noreferrer" aria-label="Facebook" title="Facebook"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M13.5 21v-8h2.7l.4-3h-3.1V8.1c0-.9.3-1.6 1.6-1.6h1.7V3.8c-.3 0-1.3-.1-2.5-.1-2.5 0-4.2 1.5-4.2 4.3V10H7.3v3h2.8v8h3.4Z"/></svg></a>
            <a class="header-social-link" href="https://www.threads.net/" target="_blank" rel="noopener noreferrer" aria-label="Threads" title="Threads"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M17.1 11.1c-.2-4.3-2.8-6.8-7-6.8-4.3 0-7.1 2.6-7.1 6.8 0 4.7 2.9 7.5 7.5 7.5 3.8 0 6.2-1.9 6.2-4.9 0-2.4-1.8-4-4.6-4-2.2 0-3.7 1-3.7 2.6 0 1.3 1.1 2.1 2.6 2.1 1.6 0 2.8-.8 3.5-2.1-.3 2.1-1.5 3.3-3.8 3.3-3.5 0-5.5-1.7-5.5-4.7 0-3 1.8-4.7 4.9-4.7 2.8 0 4.5 1.2 5 3.5-1.3-.5-2.6-.7-3.8-.7-1.8 0-2.8.7-2.8 1.9-1.8 0-2.8.7-2.8 1.9 0 .8.7 1.3 1.7 1.3 1.7 0 3-1.2 3.1-3.1.1 0 .2 0 .3.1 1.7.3 2.7 1.2 2.7 2.7 0 1.9-1.6 3-4.1 3-3.8 0-6.2-2.2-6.2-5.9 0-3.5 2.2-5.7 5.6-5.7 3.1 0 5.1 1.6 5.6 4.4l2.2.1c-.1-.3-.2-.6-.3-.9Z"/></svg></a>
            ...
            <a class="header-social-link" href="mailto:dulichannle@gmail.com" aria-label="Email AnnLeTravel" title="Email"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 6.2A2.2 2.2 0 0 1 4.7 4h14.6a2.2 2.2 0 0 1 2.2 2.2v11.6a2.2 2.2 0 0 1-2.2 2.2H4.7a2.2 2.2 0 0 1-2.2-2.2V6.2Zm2.2-.1 7.3 5.7 7.3-5.7H4.7Zm14.6 2.8-6.6 5.1a1.1 1.1 0 0 1-1.4 0L4.7 8.9v8.9h14.6v-8.9Z"/></svg></a>`;

        if (actions) {
            const audienceSwitch = actions.querySelector(".audience-switch-btn");
            const consult = actions.querySelector(".btn-header");
            if (audienceSwitch) {
                audienceSwitch.insertAdjacentElement("afterend", social);
            } else if (consult) {
                consult.insertAdjacentElement("beforebegin", social);
            } else {
                actions.appendChild(social);
            }
        } else {
            const consult = container.querySelector(".btn-header");
            if (consult) consult.insertAdjacentElement("beforebegin", social);
            else container.appendChild(social);
        }
    }

    function injectHeaderSocialStyles() {
        if (document.getElementById("annle-header-social-styles")) return;
