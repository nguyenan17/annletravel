 ...
 * Keeps navigation consistent across personal/business pages and centralizes active states.
 */
(function () {
  ...
  function destinationDropdown(active) {
    return '<div class="nav-dropdown"><a href="destinations.html"' + (active ? ' class="active"' : '') + '>Điểm đến <span class="nav-chevron">▾</span></a><ul id="destinationMenu" class="dropdown-menu"></ul></div>';
  }
  ...
  function renderHeader() {
    ...
    if (isBusiness) {
      ...
      menu += '<a href="business.html#businessServices">Giải pháp</a>';
      menu += destinationDropdown(isDestinations);
      menu += '<a href="business.html#businessProcess">Quy trình</a>';
    ...
    var headerClass = isBusiness ? 'header audience-header business-shared-header' : 'header audience-header personal-shared-header';
    var switchLabel = isBusiness ? 'Cá nhân' : 'Doanh nghiệp';
    var switchHref = isBusiness ? 'ca-nhan.html' : 'business.html';

    existing.className = headerClass;
    existing.innerHTML = '<div class="container header-container">' +
