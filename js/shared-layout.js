...
(function () {
    ...
    function renderLanguageSwitcher() {
        return `<div class="language-switcher" data-annle-shared-language-switcher aria-label="Language">
            ...
            <div class="language-options" role="menu">
                ${Object.entries(LANGUAGES).map(([lang, item]) => `<button type="button" data-shared-lang="${lang}" role="menuitem" aria-label="${item.name}"><span class="language-option-flag" aria-hidden="true">${item.flag}</span></button>`).join("")}
            </div>
        </div>`;
    }
    ...
    function setLanguageUi(lang) {
        ...
        if (!box) return;
        const item = LANGUAGES[selected];
        const currentFlag = box.querySelector(".language-flag");
        if (currentFlag) {
            currentFlag.textContent = item.flag;
            currentFlag.style.backgroundImage = "none";
        }
