...
(function () {
    ...
    if (header) {
        ...
        header.innerHTML = `<div class="container header-container"><a href="${homeHref}" class="logo"><img src="images/logo.png" alt="ANNLETRAVEL"></a><nav class="menu">${menu}</nav><div class="public-header-actions"><div class="language-switcher" aria-label="Language"><button type="button" class="language-current" aria-expanded="false" aria-haspopup="true"><span class="language-flag" aria-hidden="true"></span><span class="language-code">VI</span><span class="language-arrow">▾</span></button><div class="language-options" role="menu"><button type="button" data-lang-switch="vi"><span class="language-option-flag"></span><span>Tiếng Việt</span></button><button type="button" data-lang-switch="en"><span class="language-option-flag"></span><span>English</span></button></div></div><a href="${switchHref}" class="audience-switch-btn">${switchLabel}</a><a href="${contactHref}" class="btn btn-header" data-i18n="nav.consult">Tư vấn ngay</a></div></div>`;
        ...
        const setLanguageUi = lang => {
            ...
            header.querySelectorAll("[data-lang-switch]").forEach(button => button.classList.toggle("active", button.dataset.langSwitch === (selected === "EN" ? "en" : "vi")));
        };
        ...
        header.querySelectorAll("[data-lang-switch]").forEach(button => button.addEventListener("click", event => {
            event.preventDefault();
            const lang = button.dataset.langSwitch === "en" ? "en" : "vi";
            if (window.AnnLeI18n?.applyLanguage) window.AnnLeI18n.applyLanguage(lang);
            else { localStorage.setItem("annletravel_language", lang); window.location.reload(); }
