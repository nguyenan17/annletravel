...
(function () {
  ...
  function formatCampaignPrice(v){return Number(v||0).toLocaleString('vi-VN')+'đ';}
  function active(){var now=new Date();return arguments.length?[]:[];}
    ...
    function renderPromotions(data){
        ...
        host.innerHTML=items.slice(0,3).map(function(t){return '<article class="campaign-tour-card"><a class="campaign-tour-image" href="tour-detail.html?id='+encodeURIComponent(t.id)+'" style="background-image:url(\\''+esc(t.image||'')+'\\')"><span class="campaign-badge">🔥 ƯU ĐÃI CÓ HẠN</span></a><div class="campaign-tour-body"><p class="campaign-tour-destination">'+esc(t.destination)+'</p><h3>'+esc(t.name)+'</h3><p>'+esc(t.short)+'</p><div class="campaign-tour-bottom"><strong>'+(t.sale_price?formatCampaignPrice(t.sale_price):formatCampaignPrice(t.price))+'</strong><a href="tour-detail.html?id='+encodeURIComponent(t.id)+'">Xem tour →</a></div></div></article>';}).join('');
  ...
  function renderExperiences(){
    ...
    if(!host)return;
    var items=window.__annleCampaignTours||[];
    var experienceItems=items.filter(function(t){return t.experience_enabled;}).slice(0,3);
    host.innerHTML=experienceItems.map(function(t){
      return '<a class="experience-card" href="tour-detail.html?id='+encodeURIComponent(t.id)+'"><span class="experience-icon">🌿</span><div><h3>'+esc(t.name)+'</h3><p>'+esc(t.short)+'</p><span>Xem hành trình →</span></div></a>';
    }).join('');
  }
